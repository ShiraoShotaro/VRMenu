...
namespace wproj
{
	...
	public class SteamVROverlayer : MonoBehaviour
	{
		//エラーメッセージの名前
		const string Tag = "[SteamVROverlayer]";

		//グローバルキー(システムのオーバーレイ同士の識別名)。
		//ユニークでなければならない。乱数やUUIDなどを勧める
		string OverlayKey = "SteamVROverlayer";

		//ユーザーが確認するためのオーバーレイの名前
		string OverlayFriendlyName = "SteamVROverlayer";
		...
		SteamVROverlayer()
		{
			OverlayKey = new System.Guid().ToString();
		}

		void Awake()
		{
			OverlayFriendlyName = Application.productName;
			...
			{
				Debug.LogError(Tag + "Failed to initialzie OpenVR. Error = " + openVRError.ToString());
				ApplicationQuit();
				return;
			}
			...
			{
				Debug.LogError(Tag + "Failed to initialize SteamVROverlay. Error = " + overlayError.ToString());
				ApplicationQuit();
				return;
			}
			...
			Debug.Log(Tag + "Finished initializing SteamVROverlay.");
		}
		...
		void Update()
		{
			...
			if (overlay.IsOverlayVisible(overlayHandle))
			{
				...
				pose.m0 = 1; pose.m1 = 0; pose.m2 = 0; pose.m3 = posX;
				pose.m4 = 0; pose.m5 = textureYflip; pose.m6 = 0; pose.m7 = posY;
				pose.m8 = 0; pose.m9 = 0; pose.m10 = 1; pose.m11 = posZ;
